...
class qqAPI():
	...
	def t_qq_search_by_keyword(self, keyword="", page=1):
		RESOURCE_URL = self.API_ROOT + "/search/tweet"
		parameters = {
			'format': 'json',
			'keyword': keyword,
			'pagesize': 20,
		...
		return simplejson.loads(self.access_response(RESOURCE_URL, parameters))['data']['info']
	...
	def t_qq_search_by_user(self, name="", id="",page=1):
		...
		return simplejson.loads(self.access_response(RESOURCE_URL, parameters))['data'].get('info')
